...
public class GameUI extends ConsoleHost implements Console.Directory {
        ...
        add(new Widget(new Coord(360, 40)) {
            ...
            public void draw(GOut g) {
                if (Config.showservertime) {
                    Tex time = ui.sess.glob.servertimetex; //might have issue
                    if (time != null)
                        g.image(time, new Coord(360 / 2 - time.sz().x / 2, 0));
                }
            }
        }, new Coord(JOGLPanel.lshape.sz().x / 2 - 360 / 2, umpanel.sz.y));
    ...
    public void addchild(Widget child, Object... args) {
    	...
    	if(place == "mapview") {
            ...
            minimapWnd = minimap();
    	    mmap = minimapWnd.mmap;//blpanel.add(new LocalMiniMap(new Coord(133, 133), map), minimapc);
    	    //mmap.lower();
    	    ...
    	    if(mapstore != null) {
                ...
                mapfile.hide();
        		add(mapfile,50, 50); // Utils.getprefc("wndc-map", new Coord(50, 50)));
                minimapWnd.mapfile = mapfile;
    	    }
    	} else if(place == "menu") {
    	    menu = (MenuGrid)brpanel.add(child,  20, 34);//menugridc);
    	} else if(place == "fight") {
    ...
    public void resize(Coord sz) {
    	this.sz = sz;
    	chat.resize(Config.chatsz);//sz.x - blpw - brpw);
